...
class LocalAudioService implements AudioService {
  ...
  async list(filters?: AudioFilters): Promise<AudioRecord[]> {
    ...
    if (filters?.entryId) {
      records = records.filter(
        (r: AudioRecord) => r.metadata.entryId === filters.entryId
      );
    }
    if (filters?.level) {
      records = records.filter(
        (r: AudioRecord) => r.metadata.level === filters.level
      );
    }
    if (filters?.levelId) {
      records = records.filter(
        (r: AudioRecord) => r.metadata.levelId === filters.levelId
      );
    }
    if (filters?.speaker) {
      records = records.filter(
        (r: AudioRecord) => r.metadata.speaker === filters.speaker
      );
    }
    if (filters?.dialect) {
      records = records.filter(
        (r: AudioRecord) => r.metadata.dialect === filters.dialect
      );
    }

    return records.sort(
      (a: AudioRecord, b: AudioRecord) =>
  ...
  async getStats() {
    ...
    const entriesWithAudio = new Set(
      records.map((r: AudioRecord) => r.metadata.entryId)
    ).size;
  ...
  private async removeFromIndex(audioId: string) {
    ...
    if (record) {
      ...
      if (index.entryIndex[entryId]) {
        index.entryIndex[entryId] = index.entryIndex[entryId].filter(
          (id: string) => id !== audioId
        );
      ...
      const level = record.metadata.level;
      if (index.levelIndex[level]) {
        index.levelIndex[level] = index.levelIndex[level].filter(
          (id: string) => id !== audioId
        );
