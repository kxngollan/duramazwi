...
import { conversationChallenges } from "@/components/challenge/conversation-challenges";
...
> = {
  easy: {
    label: "Easy",
    description: "16 beginner challenges. Key phrase identification across all five settings.",
    difficulty: "beginner",
    count: 16,
    icon: <IconStar />,
  },
  medium: {
    label: "Medium",
    description: "16 intermediate challenges. Context-appropriate responses and conversation sequencing.",
  ...
  hard: {
    label: "Hard",
    description: "8 advanced challenges. Build full Shona conversational exchanges.",
    difficulty: "advanced",
    count: 8,
    icon: <IconBolt />,
  },
  mixed: {
    label: "Mixed",
    description: "10 random challenges across all difficulties and settings.",
    count: 10,
    icon: <IconSquares />,
  },
};

function prepareChallenge(raw: (typeof conversationChallenges)[0]): Challenge {
  return {
    id: raw.id,
...
function buildSet(mode: Mode): Challenge[] {
  ...
  const pool = cfg.difficulty
    ? conversationChallenges.filter((c) => c.difficulty === cfg.difficulty)
    : shuffleArray([...conversationChallenges]);
...
export default function ConversationChallengesPage() {
